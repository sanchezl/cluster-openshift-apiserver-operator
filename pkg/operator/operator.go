...
func NewKubeApiserverOperator(
	...
	kubeInformersLocallyNamespaced kubeinformers.SharedInformerFactory,
	kubeInformersEtcdNamespaced kubeinformers.SharedInformerFactory,
	...
	kubeInformersEtcdNamespaced.Core().V1().ConfigMaps().Informer().AddEventHandler(c.eventHandler())
	kubeInformersEtcdNamespaced.Core().V1().ServiceAccounts().Informer().AddEventHandler(c.eventHandler())
	kubeInformersLocallyNamespaced.Core().V1().ConfigMaps().Informer().AddEventHandler(c.eventHandler())
	kubeInformersLocallyNamespaced.Core().V1().ServiceAccounts().Informer().AddEventHandler(c.eventHandler())
	kubeInformersLocallyNamespaced.Core().V1().Services().Informer().AddEventHandler(c.eventHandler())
	kubeInformersLocallyNamespaced.Apps().V1().Deployments().Informer().AddEventHandler(c.eventHandler())
	apiregistrationInformers.Apiregistration().V1().APIServices().Informer().AddEventHandler(c.eventHandler())

	// we only watch some namespaces
	kubeInformersLocallyNamespaced.Core().V1().Namespaces().Informer().AddEventHandler(c.namespaceEventHandler())
